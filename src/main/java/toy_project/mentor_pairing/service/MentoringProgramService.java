...
public class MentoringProgramService {
    ...
    public Long match(Long menteeApplicationId, Long mentorApplicationId){
        ...
        //3. 두 신청이 존재하는지 확인
        if(menteeApplication == null) throw new MentoringException("멘티신청이 존재하지 않습니다.");
        if(mentorApplication == null) throw new MentoringException("멘토신청이 존재하지 않습니다.");
        //4. 각각 MENTEE, MENTOR 역할 확인
        if(menteeApplication.getRole()!= MentoringRole.MENTEE) throw new MentoringException("신청 역할과 멘티가 일치하지 않습니다.");
        if(mentorApplication.getRole()!= MentoringRole.MENTOR) throw new MentoringException("신청 역할과 멘토가 일치하지 않습니다.");
        //5. 두 신청 모두 Pending인지 확인
        if(menteeApplication.getStatus() != ApplicationStatus.PENDING) throw new MentoringException("멘토링 신청 대기상태가 아닙니다.");
        if(mentorApplication.getStatus() != ApplicationStatus.PENDING) throw new MentoringException("멘토링 신청 대기상태가 아닙니다.");
        //6. 신청 과목이 같은지 확인
        if(mentorApplication.getSubject() != menteeApplication.getSubject()) throw new MentoringException("신청 과목이 서로 다릅니다.");
        ...
        if(!menteeNowActiveMentoringList.isEmpty()) throw new MentoringException("이미 진행 중인 멘토링이 있습니다.");
    ...
    private static @NonNull MentoringProgram getMentoringProgram(List<MentorRecommendation> mentorRecommendationList, MentoringApplication mentorApplication, MentoringApplication menteeApplication) {
        ...
        if(selectedMentorRecommendation == null) throw new MentoringException("추천 멘토에 신청 멘토가 존재하지 않습니다.");
